...
$(document).ready(function () {
    // Asegura que el último elemento del .scrollable-content tenga un relleno inferior suficiente
    var lastCard = $(".scrollable-content .card").last();
    lastCard.css("margin-bottom", "50px"); // Ajusta el valor según lo que necesites
});
...
function openEditModal(id, nombre, link, audiencias, tarifas) {
    // Colocar los valores en los campos del modal
    document.getElementById('editMediaId').value = id;
    document.getElementById('editNombre').value = nombre;
    document.getElementById('editLink').value = link;
    document.getElementById('editAudiencias').value = audiencias;
    document.getElementById('editTarifas').value = tarifas;
...
document.addEventListener("DOMContentLoaded", function() {
    // Esto se ejecuta cuando todo el DOM ha cargado
    const deleteButtons = document.querySelectorAll(".delete-button");
    deleteButtons.forEach(button => {
        button.addEventListener("click", function() {
            const clienteId = button.dataset.clienteid;
            const palabraId = button.dataset.palabraid;
            confirmDelete(clienteId, palabraId);
        });
    });
});
